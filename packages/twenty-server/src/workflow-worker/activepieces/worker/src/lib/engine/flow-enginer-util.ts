...
export const pieceEngineUtil = {
  ...
  async extractFlowPieces({
    ...
    const pieces = steps
      .filter(
        (step) =>
          step.type === TriggerType.PIECE || step.type === ActionType.PIECE,
      )
      .map((step) => {
        const { packageType, pieceType, pieceName, pieceVersion } =
          step.settings as PieceTriggerSettings | PieceActionSettings;
    ...
    switch (packageType) {
      ...
      case PackageType.REGISTRY: {
        ...
        const version = exactVersion
          ? pieceVersion
          : (
              await engineApiService(engineToken).getPiece(pieceName, {
                version: pieceVersion,
              })
            ).version;
  ...
  async getExactPieceForStep(
    engineToken: string,
    step: Action | Trigger,
  ): Promise<PiecePackage> {
    const pieceSettings = step.settings as
      | PieceTriggerSettings
      | PieceActionSettings;
  ...
  async lockPieceInFlowVersion({
    ...
    return flowHelper.transferFlowAsync(flowVersion, async (step) => {
      ...
      if (step.type === TriggerType.PIECE) {
        const piece = await pieceEngineUtil.getExactPieceForStep(
          engineToken,
          step,
        );
