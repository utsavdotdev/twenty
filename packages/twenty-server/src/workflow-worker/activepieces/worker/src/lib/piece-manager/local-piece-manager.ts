  ...
  protected override async installDependencies(
    ...
    for (const piece of pieces) {
      assertEqual(
        ...
        'packageType',
        `Piece ${piece.pieceName} is not of type REGISTRY`,
      );
      const directoryPath = await filePiecesUtils.findDirectoryByPackageName(
        piece.pieceName,
      );

      assertNotNullOrUndefined(
        directoryPath,
        `directoryPath for ${piece.pieceName} is null or undefined`,
      );
