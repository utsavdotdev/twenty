...
import { PieceMetadataModel } from 'src/workflow-worker/activepieces/pieces-framework/src';
import {
  ApQueueJob,
  ...
  return {
    ...
    async poll(queueName: QueueName): Promise<ApQueueJob | null> {
      try {
        ...
        const response = await client.get<ApQueueJob | null>(
          '/v1/workers/poll',
          {
            params: request,
          },
...
export const engineApiService = (engineToken: string) => {
  ...
  return {
    ...
    async getPiece(
      name: string,
      options: GetPieceRequestQuery,
    ): Promise<PieceMetadataModel> {
      return client.get<PieceMetadataModel>(
        `/v1/pieces/${encodeURIComponent(name)}`,
        {
          params: options,
        },
      );
    ...
    async checkTaskLimit(): Promise<void> {
      try {
        ...
        if (
          ApAxiosClient.isApAxiosError(e) &&
          e.error.response &&
          e.error.response.status === StatusCodes.PAYMENT_REQUIRED
        ) {
