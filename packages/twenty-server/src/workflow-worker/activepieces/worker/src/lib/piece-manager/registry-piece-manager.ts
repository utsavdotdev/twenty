...
export class RegistryPieceManager extends PieceManager {
  protected override async installDependencies({
    ...
    const dependenciesToInstall = await this.filterExistingPieces(
      projectPath,
      pieces,
    );
    ...
    const installPromises = pieces.map(async (piece) => {
      const pnpmAddLock = await memoryLock.acquire(
        `pnpm-add-${projectPath}-${piece.pieceName}-${piece.pieceVersion}`,
      );

      try {
        const dependencies = await this.filterExistingPieces(projectPath, [
          piece,
        ]);
  ...
  private async filterExistingPieces(
    projectPath: string,
    pieces: PiecePackage[],
  ): Promise<PackageInfo[]> {
    const enrichedDependencies = await Promise.all(
      pieces.map(async (piece) => {
        ...
        const fExists = await fileExists(
          join(projectPath, 'node_modules', pkg.alias),
        );
