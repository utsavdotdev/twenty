...
export class IsolateSandbox {
  ...
  private getEnvironmentVariables(): Record<string, string> {
    ...
    return {
      ...
      AP_CODE_SANDBOX_TYPE: system.getOrThrow(
        SharedSystemProp.CODE_SANDBOX_TYPE,
      ),
  ...
  private getDirsToBindArgs(): string[] {
    ...
    const dirsToBind = [
      '--dir=/usr/bin/',
      `--dir=/etc/=${etcDir}`,
      `--dir=${IsolateSandbox.sandboxGlobalCachePath}=${path.resolve(
        globalCachePath,
      )}`,
    ];
    ...
    if (this._customPiecesPath) {
      dirsToBind.push(
        `--dir=${IsolateSandbox.sandboxCustomPiecesCachePath}=${path.resolve(
          this._customPiecesPath,
          'node_modules',
        )}`,
      );
    }
    if (this._flowVersionId) {
      dirsToBind.push(
        `--dir=${path.join(
          IsolateSandbox.sandboxCodesCachePath,
          this._flowVersionId,
        )}=${path.resolve(globalCodesCachePath, this._flowVersionId)}`,
      );
    }

    const piecesSource = system.getOrThrow<PiecesSource>(
      SharedSystemProp.PIECES_SOURCE,
    );
