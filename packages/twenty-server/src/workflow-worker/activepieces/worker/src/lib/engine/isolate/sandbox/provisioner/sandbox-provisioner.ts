...
import {
  extractProvisionCacheKey,
  TypedProvisionCacheInfo,
} from './sandbox-cache-key';
...
export const sandboxProvisioner = {
  async provision({
    ...
    try {
      ...
      const hasAnyCustomPieces = pieces.some(
        (f) => f.pieceType === PieceType.CUSTOM,
      );
      const sandbox = await sandboxManager.allocate(cacheKey);
      const flowVersionId =
        codeSteps.length > 0 ? codeSteps[0].flowVersionId : undefined;
