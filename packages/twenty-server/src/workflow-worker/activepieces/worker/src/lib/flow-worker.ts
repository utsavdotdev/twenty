...
const WORKER_CONCURRENCY = system.getNumberOrThrow(
  WorkerSystemProps.FLOW_WORKER_CONCURRENCY,
);
const POLLING_CONCURRENCY = system.getNumberOrThrow(
  WorkerSystemProps.POLLING_CONCURRENCY,
);
...
export const flowWorker = {
  init(generatedToken: string): void {
    closed = false;
