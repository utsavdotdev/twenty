...
const executionState = FlowExecutorContext.empty()
  ...
  .upsertStep(
    ...
    GenericStepOutput.create({
      ...
      input: {},
      output:
        'memory://{"fileName":"hello.png","data":"iVBORw0KGgoAAAANSUhEUgAAAiAAAAC4CAYAAADaI1cbAAA0h0lEQVR4AezdA5AlPx7A8Zxt27Z9r5PB2SidWTqbr26S9Hr/tm3btu3723eDJD3r15ec17vzXr+Z"}',
    }),
  );
...
describe('Variable Service', () => {
  test('Test resolve text with no variables', async () => {
    const { resolvedInput } = await variableService.resolve({
      unresolvedInput: 'Hello world!',
      executionState,
    });

    expect(resolvedInput).toEqual('Hello world!');
  });
  ...
  test('Test resolve text with double variables', async () => {
    ...
    expect(resolvedInput).toEqual('Price is 6.4');
  });

  test('Test resolve object steps variables', async () => {
    const { resolvedInput } = await variableService.resolve({
      unresolvedInput: '{{trigger}}',
      executionState,
    });
  ...
  test('Test resolve variable array items', async () => {
    const { resolvedInput } = await variableService.resolve({
      unresolvedInput: '{{trigger.items[0]}} {{trigger.items[1]}}',
      executionState,
    });

    expect(resolvedInput).toEqual('5 a');
  });

  test('Test resolve array variable', async () => {
    const { resolvedInput } = await variableService.resolve({
      unresolvedInput: '{{trigger.items}}',
      executionState,
    });
  ...
  test('Test resolve integer from variables', async () => {
    const { resolvedInput } = await variableService.resolve({
      unresolvedInput: '{{trigger.items[0]}}',
      executionState,
    });

    expect(resolvedInput).toEqual(5);
  });

  test('Test resolve text with undefined variables', async () => {
    const { resolvedInput } = await variableService.resolve({
      unresolvedInput: 'test {{configs.bar}} {{trigger.items[4]}}',
      executionState,
    });

    expect(resolvedInput).toEqual('test  ');
  });
  ...
  test('Test resolve object', async () => {
    ...
    expect(resolvedInput).toEqual({
      input: { foo: 'bar', nums: [1, 2, 5], var: 6.4 },
    });
  });

  test('Test resolve boolean from variables', async () => {
    const { resolvedInput } = await variableService.resolve({
      unresolvedInput: '{{step_1.success}}',
      executionState,
    });
  ...
  test('Test resolve addition from variables', async () => {
    ...
    expect(resolvedInput).toEqual(6.4 + 2 - 3);
  });
  ...
  test('Test resolve text with array variable', async () => {
    ...
    expect(resolvedInput).toEqual('items are [5,"a"]');
  });

  test('Test resolve text with object variable', async () => {
    const { resolvedInput } = await variableService.resolve({
      unresolvedInput: 'values from trigger step: {{trigger}}',
      executionState,
    });

    expect(resolvedInput).toEqual(
      'values from trigger step: {"items":[5,"a"],"name":"John","price":6.4}',
    );
  });
  ...
  test('Test use built-in Math Min function', async () => {
    ...
    expect(resolvedInput).toEqual(2);
  });
  ...
  test('Test use built-in Math Max function', async () => {
    ...
    expect(resolvedInput).toEqual(8.4);
  });
  ...
  it('should return error for invalid data', async () => {
    ...
    expect(errors).toEqual({
      file: [
        'Expected file url or base64 with mimeType, but found value: https://google.com',
      ],
      nullFile: ['Expected value, but found value: null'],
    });
  });
  ...
  it('should return errors for invalid number', async () => {
    ...
    expect(errors).toEqual({
      ...
      undefinedNumber: ['Expected value, but found value: undefined'],
      auth: {
  ...
  it('Test email validator', async () => {
    ...
    const { errors } = await variableService.applyProcessorsAndValidators(
      input,
      props,
      PieceAuth.CustomAuth({
        required: true,
  ...
  it('Test url and oneOf validators', async () => {
    ...
    const props = {
      text: Property.LongText({
        ...
        validators: [
          Validators.url,
          Validators.oneOf(['activepieces.com', 'www.activepieces.com']),
        ],
      }),
    };
    const { errors } = await variableService.applyProcessorsAndValidators(
  ...
  it('Test minLength and maxLength validators', async () => {
    ...
    expect(errors).toEqual({
      ...
      text2: [
        'The value: short1234678923145678 may not be greater than 10 characters',
      ],
    });
  });
  ...
  it('Test maxValue, inRange, minValue and oneOf valdiators', async () => {
    ...
    const props = {
      value1: Property.Number({
        ...
        required: true,
        validators: [
          Validators.maxValue(2),
          Validators.oneOf(Object.values(choices)),
        ],
      }),
    ...
    expect(errors).toEqual({
      value1: [
        'The value: 40 must be 2 or less',
        'The 40 is not a valid value, valid choices are: 1,2',
      ],
