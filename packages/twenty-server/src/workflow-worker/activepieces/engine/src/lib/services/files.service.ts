...
export function isApFilePath(dbPath: unknown): dbPath is string {
  ...
  return (
    dbPath.startsWith(FILE_PREFIX_URL) ||
    dbPath.startsWith(DB_PREFIX_URL) ||
    dbPath.startsWith(MEMORY_PREFIX_URL)
  );
}

export async function handleAPFile({
  engineToken,
  path,
  apiUrl,
}: {
...
async function readDbFile({
  ...
  const response = await fetch(
    `${apiUrl}v1/step-files/${encodeURIComponent(fileId)}`,
    {
      method: 'GET',
      headers: {
